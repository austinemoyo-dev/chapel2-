...
const STATUS_CONFIG = {
  valid:    { label: 'Present',    color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20', dot: 'bg-emerald-400' },
  invalid:  { label: 'Incomplete', color: 'text-amber-400',   bg: 'bg-amber-500/10 border-amber-500/20',     dot: 'bg-amber-400'   },
  excused:  { label: 'Excused',    color: 'text-blue-400',    bg: 'bg-blue-500/10 border-blue-500/20',       dot: 'bg-blue-400'    },
  upcoming: { label: 'Upcoming',   color: 'text-muted',       bg: 'bg-surface-2 border-border',              dot: 'bg-muted'       },
  missed:   { label: 'Absent',     color: 'text-red-400',     bg: 'bg-red-500/10 border-red-500/20',         dot: 'bg-red-400'     },
};
        ...
        <>
          ...
          <div className="flex gap-2">
            {(['all', 'present', 'absent'] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)}
                className={`px-4 py-1.5 rounded-xl text-xs font-semibold transition-colors ${filter === f ? 'bg-primary text-white' : 'bg-surface-2 text-muted'}`}>
          ...
          <div className="space-y-2">
            {filtered.map(svc => {
              ...
              return (
                <div key={svc.service_id} className={`rounded-2xl p-4 border ${cfg.bg}`}>
