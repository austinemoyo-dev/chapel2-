    ...
    <div className="min-h-dvh bg-background px-4 py-6 space-y-5 max-w-lg mx-auto">
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3].map(i => <div key={i} className="h-24 rounded-2xl bg-surface-2 animate-pulse" />)}
        </div>
      ) : (
        <>
                  ...
                  <div key={svc.id} className="glass-panel rounded-2xl p-4 border border-border">
                    <div className="flex items-start justify-between">
                      ...
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                        svc.is_window_open ? 'bg-emerald-500/20 text-emerald-300' : 'bg-surface-2 text-muted'
                      }`}>
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                        svc.signed_in ? 'bg-emerald-500/20 text-emerald-300' : 'bg-surface-2 text-muted'
                      }`}>
                        {svc.signed_in ? '✓ Signed in' : '✗ Not scanned'}
                      </span>
                      {svc.signout_required && (
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                          svc.signed_out ? 'bg-emerald-500/20 text-emerald-300' : 'bg-amber-500/20 text-amber-300'
                        }`}>
                    ...
                    {svc.notes && (
                      <p className="text-xs text-amber-300 mt-2 bg-amber-500/10 rounded-lg px-2 py-1">📌 {svc.notes}</p>
                    )}
                  </div>
                ))}
              </div>
